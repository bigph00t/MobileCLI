...
impl RelayState {
    ...
    fn cleanup(&self) {
        ...
        for entry in self.rooms.iter() {
            ...
            if (!room.client_joined && age > ROOM_EXPIRY_NO_CLIENT) || idle > ROOM_EXPIRY_IDLE {
        ...
        self.rate_limits
            .retain(|_, entry| now.duration_since(entry.window_start) < RATE_LIMIT_WINDOW * 2);
    }
}
...
async fn main() {
    ...
    info!(
        "  Room expiry: {} min (no client) / {} min (idle)",
            ...
            tokio::select! {
                _ = heartbeat.tick() => {
                    ...
                    if let Some(room) = state.rooms.get(&code) {
                        if room.last_activity.elapsed() > HEARTBEAT_TIMEOUT {
                            info!("Host heartbeat timeout, closing room: {}", code);
                            if let Some(client_tx) = &room.client_tx {
        ...
        loop {
            tokio::select! {
                _ = heartbeat.tick() => {
                    ...
                    if let Some(mut room) = state.rooms.get_mut(&code) {
                        if room.last_activity.elapsed() > HEARTBEAT_TIMEOUT {
                            info!("Client heartbeat timeout: {}", code);
                            room.client_tx = None;
